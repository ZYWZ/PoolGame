...
int main(int argc, char *argv[])
{
    ...
    if (conf.value("stage2").toBool(false) == false && conf.value("stage3").toBool(false) == false){
        director.setBuilder(new StageOneBuilder());
    }
    else if (conf.value("stage2").toBool(false) == true && conf.value("stage3").toBool(false) == false) {
    ...
    Dialog w(game, stageFlag, nullptr);
    w.show();
