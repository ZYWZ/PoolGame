...
void Game::render(QPainter &painter) const {
    ...
    if(m_flashlight){
        this->renderFlashLight(painter);
    }
}

void Game::renderFlashLight(QPainter &painter) const {
    painter.setBrush(QBrush(QColor("black")));
    QPainterPath groupPath;
        groupPath.moveTo(0.0, 0.0);
        groupPath.lineTo(0.0, m_table->getHeight());
        groupPath.lineTo(m_table->getWidth(), m_table->getHeight());
        groupPath.lineTo(m_table->getWidth(), 0.0);
        groupPath.moveTo(0.0,0.0);
        groupPath.arcTo(m_balls->front()->getPosition().x()-flashLightRadius-m_flashRadius,
                        m_balls->front()->getPosition().y() - flashLightRadius-m_flashRadius,
                        (flashLightRadius + m_flashRadius)*2,
                        (flashLightRadius + m_flashRadius)*2,
                        135.0,
                        360.0);
...
Memento* Game::save(){
    ...
    for(auto it = m_balls->begin(); it != m_balls->end(); ++it){
        Ball* ball = *it;
        memento->pushToVector(ball->copy());
    }
    return memento;
}
